...
One approach would be to use the discretionary design pattern and create country-specific policy templates for `Compliance Officers`. In Alice's case, we would then created two template-linked policies, one usng the 1`Canada` template and the other using the `United States` template. 
