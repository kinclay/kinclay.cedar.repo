...
In Cedar, a relationship is modeled using an attribute on the resource. Each relationship gets its own attribute, which contains the set of users that have that relationship with that resource. The name of the attribute should associate it obviously with the relationship. This enables us to write Cedar policies with conditional constraints based on these attributes, as shown in the following policy:
```cedar
// Owners policy
permit (
  principal is User,
...
For example, the Tiny ToDo application, which was used in as the example for relationship permissions, might have an admin role which grants full permissions on all lists within the application; grant users the permission to mark lists as public, enabling any user to view the list, and have a housekeeping daemon that deletes old lists. 
